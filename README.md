...
![Version: 3.0.4](https://img.shields.io/badge/Version-3.0.4-informational?style=flat-square) ![AppVersion: 1.34.20_1.1.6_1.0.1](https://img.shields.io/badge/AppVersion-1.34.20_1.1.6_1.0.1-informational?style=flat-square)
...
### 2. Install
```bash
helm install datafy-agent --version 3.0.4 datafyio/datafy-agent \
...
```bash
helm repo update
helm upgrade --install datafy-agent --version 3.0.4 datafyio/datafy-agent -n <namespace> --reuse-values --atomic
```

Switch mode:
```bash
helm upgrade --install datafy-agent --version 3.0.4 datafyio/datafy-agent -n <namespace> --set agent.mode=autoscaler --atomic
```
