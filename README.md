...
![Version: 3.2.8](https://img.shields.io/badge/Version-3.2.8-informational?style=flat-square) ![AppVersion: 1.35.36_1.2.0](https://img.shields.io/badge/AppVersion-1.35.36_1.2.0-informational?style=flat-square)
...
### 2. Install
```bash
helm install datafy-agent --version 3.2.8 datafyio/datafy-agent \
...
```bash
helm repo update
helm upgrade --install datafy-agent --version 3.2.8 datafyio/datafy-agent -n <namespace> --reuse-values --atomic
```

Switch mode:
```bash
helm upgrade --install datafy-agent --version 3.2.8 datafyio/datafy-agent -n <namespace> --set agent.mode=autoscaler --atomic
```
