...
![Version: 3.3.6](https://img.shields.io/badge/Version-3.3.6-informational?style=flat-square) ![AppVersion: 1.35.6_1.3.0](https://img.shields.io/badge/AppVersion-1.35.6_1.3.0-informational?style=flat-square)
...
### 2. Install
```bash
helm install datafy-agent --version 3.3.6 datafyio/datafy-agent \
...
```bash
helm repo update
helm upgrade --install datafy-agent --version 3.3.6 datafyio/datafy-agent -n <namespace> --reuse-values --atomic
```

Switch mode:
```bash
helm upgrade --install datafy-agent --version 3.3.6 datafyio/datafy-agent -n <namespace> --set agent.mode=autoscaler --atomic
```
