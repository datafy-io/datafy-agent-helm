...
![Version: 2.0.7](https://img.shields.io/badge/Version-2.0.7-informational?style=flat-square) ![AppVersion: 1.2.7_1.1.2](https://img.shields.io/badge/AppVersion-1.2.7_1.1.2-informational?style=flat-square)
...
### 2. Install
```bash
helm install datafy-agent --version 2.0.7 datafyio/datafy-agent \
...
```bash
helm repo update
helm upgrade --install datafy-agent --version 2.0.7 datafyio/datafy-agent -n <namespace> --reuse-values --atomic
```

Switch mode:
```bash
helm upgrade --install datafy-agent --version 2.0.7 datafyio/datafy-agent -n <namespace> --set agent.mode=autoscaler --atomic
```
